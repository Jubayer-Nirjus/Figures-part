...
tier_sizes = {0:90, 1:200, 2:450, 3:750} # Slighly increased bubble scaling for crisp high-res tiff output
tier_labels = {0:'Not reported (\u25c6 marker)',1:'<85%',2:'85\u201394%',3:'\u226595%'}

fig, ax = plt.subplots(figsize=(15, 8))

np.random.seed(42)
for year, sp_y, perf, model in studies:
    jx = np.random.uniform(-0.14, 0.14)
    jy = np.random.uniform(-0.12, 0.12)
    if perf == 0:
        ax.scatter(year+jx, sp_y+jy,
                   s=tier_sizes[0],
                   c=model_colors[model],
                   alpha=0.88, edgecolors='white', linewidths=1.2,
                   marker='D', zorder=3)
    else:
        ax.scatter(year+jx, sp_y+jy,
                   s=tier_sizes[perf],
...
ax.set_xlabel('Publication year', fontsize=12, labelpad=10)
ax.set_xlim(2014, 2026)
ax.set_xticks(range(2015, 2026))
ax.set_xticklabels([str(y) for y in range(2015, 2026)], fontsize=10.5)
...
# Horizontal species separator boundaries
for yv in [0.5, 1.5, 2.5]:
    ax.axhline(yv, color='#CCCCCC', linewidth=0.8, zorder=0)

# Legend 1: Model Family (Shifted safely to the right panel to avoid plotting area coverage)
model_handles = [
...
leg1 = ax.legend(handles=model_handles,
                 title='AI model family', title_fontsize=9.5,
                 fontsize=9, loc='upper left', bbox_to_anchor=(1.02, 1.0),
                 framealpha=0.92, edgecolor='#cccccc')
ax.add_artist(leg1)

# Legend 2: Performance Tier (Stacked perfectly below Legend 1 on the right panel)
tier_handles = [
    mlines.Line2D([], [], marker='o', color='w', markerfacecolor='#595959',
                  markersize=np.sqrt(tier_sizes[t])/2.1, linestyle='None',
                  label=tier_labels[t])
    for t in [1, 2, 3]
]
tier_handles.insert(0, mlines.Line2D([], [], marker='D', color='w', markerfacecolor='#595959',
                                    markersize=7, linestyle='None', label=tier_labels[0]))

ax.legend(handles=tier_handles,
          title='Best performance metric tier', title_fontsize=9.5,
          fontsize=9, loc='upper left', bbox_to_anchor=(1.02, 0.45),
          framealpha=0.92, edgecolor='#cccccc')
...
# Balanced layout formatting preventing text and legend cutoffs
plt.tight_layout()
...
plt.close()
print("Supplementary Fig S2 saved successfully with a clean sidebar legend layout!")