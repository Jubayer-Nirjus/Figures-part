...
p2 = ax.barh(y, near_rt,   color=MID,    height=w, label='Near real-time',     alpha=0.92,
             left=real_time)
p3 = ax.barh(y, offline,   color=LIGHT,  height=w, label='Offline / batch',    alpha=0.92,
             left=[a+b for a,b in zip(real_time,near_rt)])
p4 = ax.barh(y, poc,       color=ORANGE, height=w, label='Proof-of-concept',   alpha=0.92,
             left=[a+b+c for a,b,c in zip(real_time,near_rt,offline)])
p5 = ax.barh(y, not_stated,color=GRAY,   height=w, label='Not stated',         alpha=0.7,
             left=[a+b+c+d for a,b,c,d in zip(real_time,near_rt,offline,poc)])

for i, (tot,) in enumerate(zip(totals,)):
    ax.text(tot + 0.2, i, f'n = {tot}',
            va='center', fontsize=9.5, fontweight='bold')

ax.set_yticks(y)
ax.set_yticklabels(settings, fontsize=10)
ax.invert_yaxis()
ax.set_xlabel('Number of studies', fontsize=11, labelpad=6)
ax.set_xlim(0, 27)
...
# Annotation: no outdoor real-time
ax.annotate('No outdoor system\nachieved real-time\nperformance',
            xy=(2.2, 1), xytext=(9, 1.8),
